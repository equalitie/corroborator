...
from corroborator_app.models import incident,crime_category,actor,bulletin,time_info,location,source,status_update,role,labeling,source_type,media,comment,predefined_search,actor_relationship
...
def index(request, *args, **kwargs):
    ...
    if request.user.is_authenticated():
        ...
        relation_status_set = []
        rls = actor_relationship.RELATION
        for r in rls:
        ...
        return render(request,'search.html',{'role_status_set':role_status_set,'relation_status_set':relation_status_set,'sources_set':sources_set,'labels_set':labels_set,'crimes_set':crimes_set,'status_set':status_set,'users_set':users_set,'loc_set':loc_set,'username':username,'userid':userid,'ps_incident_list':ps_incident_list,'ps_bulletin_list':ps_bulletin_list,'ps_actor_list':ps_actor_list})
    else:
...
def lookup_bulletin(request, bulletin_id,mode):
    ...
    if 'edit' in mode:
        ...
        return render_to_response('bulletin_new.html',{'username':username,'userid':userid,'time_string':time_string,'bulletin_result':bulletin_result,'status_set':status_set,'sources_set':sources_set,'labels_set':labels_set,'users_set':users_set,'media_set':media_set},RequestContext(request))
    elif 'new' in mode:
        return render_to_response('bulletin_new.html',{'username':username,'userid':userid,'status_set':status_set,'sources_set':sources_set,'labels_set':labels_set,'users_set':users_set},RequestContext(request))
    elif mode == 'save':
        if request.method == "POST" and request.is_ajax():
            element_data = json.loads(request.raw_post_data)
        ...
        return render_to_response('incident_multi_save_result.html', {'incident_result':'success'},RequestContext(request))
...
def lookup_incident(request, incident_id,mode):
    ...
    if 'edit' in mode:
        ...
        return render_to_response('incident_view.html', {'incident_result':'success'},RequestContext(request))
        ...
        incident_result = get_object_or_404(incident, pk=incident_id)
        return render_to_response('incident_view.html', {'incident_result':incident_result},RequestContext(request))
...
def lookup_actor(request, actor_id,mode):
    ...
    if 'edit' in mode:
        ...
        return render_to_response('actor_new.html', {'relation_status_set':relation_status_set,'actor_result':actor_result,'years':years,'days':days,'months':months,'locations_set':locations_set},RequestContext(request))
    elif 'new' in mode:
        ...
        username = request.user.username
        userid = request.user.id
        return render_to_response('actor_new.html',{'relation_status_set':relation_status_set,'years':years,'days':days,'months':months,'locations_set':locations_set,'username':username,'userid':userid},RequestContext(request))
    elif mode == 'multisave':
        ...
        for a in actorData['relate_actors']:
            relation_local = actor_relationship(relation_status=a['status_en'],actor_a_id=actor_result.id,actor_b_id=int(a['id']))
            relation_local.save()

        return render_to_response('actor_new.html',{'success':'Actors saved successfully'},RequestContext(request))
    elif mode == 'save':
        ...
        return render_to_response('actor_view.html', {'actor_result':actor_result,'bulletins':bulletin_result,'incidents':incident_result,'actor_relations':actor_relations},RequestContext(request))
        ...
        incident_result = incident.objects.filter(actors_role__in=roles)
        bulletin_result = bulletin.objects.filter(actors_role__in=roles)
        return render_to_response('actor_view.html', {'actor_result':actor_result,'bulletins':bulletin_result,'incidents':incident_result},RequestContext(request))