...
corrob_util.insert_new_media =function(doc){
  var current = corrob_util.$('.current > a > span > span').html();
  media_node = corrob_util.template_media(doc);
        appendClass = current.toLowerCase().slice(0,-1)+'-media'; 
  corrob_util.$('.is-media > .'+appendClass).prepend(media_node);
}
corrob_util.template_media = function(doc){
var trimmedId = doc.id;
var current = corrob_util.$('.current > a > span > span').html().toLowerCase().slice(0,-1);
        var ouput = '<li class="medium" id="'+current +'_media_'+trimmedId+'">'+
...
corrob_util.loadVideoPlayer = function(media_data){
  video_url = gl_s3_aws_media_url + media_data;
var current = corrob_util.$('.current > a > span > span').html().toLowerCase().slice(0,-1);
...
if(!corrob_util.$('.'+current.toLowerCase().slice(0,-1)+'-overlay-annotate').hasClass('hidden')){
        ...
        corrob_util.$('.elements-'+currentSearch+'s-'+current.toLowerCase()+annotate).prepend(copyBulletin);
...
var output = '<li id="'+current.toLowerCase().slice(0,-1)+'_'+search.toLowerCase()+annotate+'_'+trimmedId+'">'+
'                       <div class="content">'+
...
'</div>'+
'<div class="status">';
if(data['most_recent_status_'+currentSearch.toLowerCase()+'_exact'] != undefined){
  output += '<span class="value">'+data['most_recent_status_'+currentSearch.toLowerCase()+'_exact']+'</span>';
}
output += '</div>'+
'</div>'+
...
output +=      '</div>'+
  ...
  '<td class="is-status">'+
    '<span class="status">';
if(doc['most_recent_status_'+current.toLowerCase()+'_exact'] != undefined){
        output += '<span class="value">'+doc['most_recent_status_'+current.toLowerCase()+'_exact']+'</span>';
}
...
corrob_util.saveActor = function(mode){
  ...
  dataToSave.media_id = (corrob_util.$('[id^="actor_media_"]').length > 0)?corrob_util.$('[id^="actor_media_"]').attr('id').split('_')[2]:'';
  ...
  dataToSave.current_location =  corrob_util.$('#actor_lives_in_hidden').val();
  ...
  dataToSave.nationality_en = corrob_util.$('#actor_nationality_en').val();
  ...
  if(mode == 'annotate'){   
    ...
    var items = corrob_util.$('#'+current+'s-table').find('.is-selector > input:checked');
          items.each(function(){
                  dataToSave.update_actors.push(corrob_util.$(this).closest('.Actor').attr('id').split('_')[1]);
          });
  }
  dataToSave.id = id;
...
corrob_util.save_element_multi = function(mode){
  ...
  var actors = new Array();
        corrob_util.$('[id^="'+current+'_actor_annotate"]').each(function(){
                actors.push({'status_en':corrob_util.$(this).find('.value').html(),'id':corrob_util.$(this).attr('id').split('_')[3]});
        });

  var incidents = new Array();
                corrob_util.$('[id^="'+current+'_incident_annotate"]').each(function(){
                        incidents.push(corrob_util.$(this).attr('id').split('_')[3]);
                });
  var bulletins = new Array();
        corrob_util.$('[id^="'+current+'_bulletin_annotate"]').each(function(){
                bulletins.push(corrob_util.$(this).attr('id').split('_')[3]);
        });
  if(current == 'incident'){
  ...
  corrob_util.$('[id^="'+current+'_comment"]:not(".is-new")').each(function(){
    localCommentObj = {};
    commentData = corrob_util.$(this).data('comment-data');
    localCommentObj['assigned_user'] = commentData['userid'];
    localCommentObj['status_id'] = commentData['statusid'];
    
    localCommentObj['comment_created'] = new Date(corrob_util.$(this).find('.created').html().split(' ')[0]);
  ...
  if(mode == 'bulletin'){
        ...
        var user = corrob_util.$('[id^="user-ac-'+current+'"]').attr('id').split('_')[2];
  var confidence_score = corrob_util.$('#'+current+'_confidence_score').html();
  dataToSave.relate_actors = actors;
  dataToSave.relate_bulletins = bulletins;
  dataToSave.relate_incidents = incidents;
  ...
  corrob_util.$('#main_spinner').removeClass('hidden');
      ...
      corrob_util.clear_annotate(current);
      corrob_util.$('#main_spinner').addClass('hidden');
      corrob_util.$('#'+current+'-annotate_action_result').removeClass('hidden');
        });

}
corrob_util.clear_annotate = function(current){
  if(current == 'bulletin'){
    corrob_util.$('#'+current+'-annotate_action_result').find('li.source:not(".is-new")').remove();
  }else if(current == 'incident'){
    corrob_util.$('#'+current+'-annotate_action_result').find('li.crime:not(".is-new")').remove();
  }
  if(current == "actor"){
    corrob_util.$('#actor_sex_en').html('Sex');
    corrob_util.$('#actor_age_en').html('Age');
    corrob_util.$('#actor_occupation_en').val('');
    corrob_util.$('#actor_position_en').val('');
    corrob_util.$('#actor_ethnicity_en').val('');
    corrob_util.$('#actor_religion_en').val('');
    corrob_util.$('#actor_spoken_dialect_en').val('');
  }else{
    
    corrob_util.$('#view-placeholder-annotate-'+current).find('.elements > li').remove();
    corrob_util.$('#'+current+'-annotate_action_result').find('li.ccomment:not(".is-new")').remove();
    corrob_util.$('#'+current+'-annotate_action_result').find('li.tag:not(".is-new")').remove();
  } 
}
corrob_util.save_element = function(mode){
  ...
  corrob_util.$('[id^="'+current+'_event"]:not(".is-new")').each(function(){
    ...
    localEventObj['from'] = new Date(corrob_util.$(this).find('.'+current+'_event_from').html());
    localEventObj['to'] = new Date(corrob_util.$(this).find('.'+current+'_event_to').html());
    events.push(localEventObj);
  });
  var media = new Array();
  corrob_util.$('[id^="'+current+'_media_"]').each(function(){
    media.push(corrob_util.$(this).attr('id').split('_')[2]);   
  });
  var actors = new Array();
  corrob_util.$('[id^="'+current+'_actor_"]').each(function(){
    actors.push({'status_en':corrob_util.$(this).find('.value').html(),'id':corrob_util.$(this).attr('id').split('_')[2]});
  ...
  var new_comments = new Array();
  corrob_util.$('[id^="'+current+'_comment"]:not(".is-new"):not(".comment-noneditable")').each(function(){
                  localCommentObj = {};
      comment_data = corrob_util.$(this).data('comment-data');
                  localCommentObj['assigned_user'] = comment_data['userid'];
      localCommentObj['statusid'] = comment_data['statusid'];
...
corrob_util.loadEditableOverlay = function(data,mode){
  ...
  if(type != 'actor'){
    ...
    corrob_util.$('.do-edit').click(function(){
      ...
      corrob_util.$('.do-addComment').click(function(event){
        event.stopImmediatePropagation();
        var current = corrob_util.$('.current > a > span > span').html().toLowerCase().slice(0,-1);
        var newComment = {};
        newComment["comment"] = corrob_util.$(this).closest('.comments').find('.comment-comment').val();
              var statusid = (corrob_util.$('#'+current+'_status').val() != 'Select status')?corrob_util.$('#'+current+'_status').val():'';
        var status = corrob_util.$('#'+current+'_status option:selected').html()
        var user =corrob_util.$('[id^="user-ac-'+current+'"]').val();
        newComment["user"] = gl_username;
        newComment["userid"] = gl_userid;
        newComment["statusid"] = statusid;
...
/*        var current = corrob_util.$('.current > a > span > span').html().toLowerCase().slice(0,-1);
      ...
      var createdString = now.getFullYear() +'-'+("0" + (now.getMonth() + 1)).slice(-2)+'-'+("0" + now.getDate()).slice(-2)+' '+now.getHours()+':'+(now.getMinutes()<10?'0':'') + now.getMinutes()+':'+now.getSeconds();
       var output =   '<li id="'+current+'_comment_'+newComment['id']+'" class="comment">'+
          '<div class="actions">'+
         ...
         '</div>'+
          ...
          '<span class="'+current+'_comment_status"> - '+newComment['status']+'</span>'+
            '</div>'+
          '</div>'+
          '<div class="clearer">&nbsp;</div>'+
        '</li>';
          ...
          corrob_util.$('#'+type+'_status').val(comment_data['statusid']);
  ...
  if(type=='actor' || mode =='annotate'){
    ...
    corrob_util.$('#actor_lives_in').autocomplete({source: gl_ac_locations_list,select: function(event, ui){ corrob_util.$('#actor_lives_in_hidden').val(ui.item.id);}});
    }
    ...
    corrob_util.$('#view-placeholder-bulletin,#view-placeholder-incident,#view-placeholder-actor,#view-placeholder-annotate-bulletin,#view-placeholder-annotate-incident,#view-placeholder-annotate-actor').on('click','.do-remove',function(){ corrob_util.$(this).closest('li').remove() });
...
corrob_util.$('.do-hide-annotate').click(function(){
  var current = corrob_util.$('.current > a > span > span').html().toLowerCase().slice(0,-1);
  corrob_util.$('#confirm_dialog').html('Are you certain that you wish to close this '+current+'?');
  corrob_util.$('#confirm_dialog').dialog({
    title : 'Quite editing '+current,
    buttons : {
      "Confirm" : function (){
 ...
 corrob_util.$('.'+current+'-overlay-annotate').removeClass('is-middle');
...
corrob_util.$('.do-hide').click(function(){
  var current = corrob_util.$('.current > a > span > span').html().toLowerCase().slice(0,-1);
  corrob_util.$('#confirm_dialog').html('Are you certain that you wish to close this '+current+'?');
  if(corrob_util.$('#'+current+'-action_save').hasClass('hidden')){
    corrob_util.$('#view-placeholder-'+current).empty();
...
corrob_util.$('#annotate-bulletins-selected, #annotate-actors-selected, #annotate-incidents-selected').click(function(){
  var current = corrob_util.$('.current > a > span > span').html().toLowerCase().slice(0,-1);
  if(parseInt(corrob_util.$('#number-'+current+'s-selected').html()) == 0){
                corrob_util.$('#confirm_dialog').html('Please select some '+current+'s.');
                corrob_util.$('#confirm_dialog').dialog({
                        title : 'No '+current+'s selected.',
                        buttons : {
                                "Confirm" : function (){
                                        corrob_util.$('#confirm_dialog').dialog("close");
                                },
...
corrob_util.$('#delete_bulletins, #delete_actors, #delete_incidents').click(function(){
  var current = corrob_util.$('.current > a > span > span').html().toLowerCase();
  if(parseInt(corrob_util.$('#number-'+current+'-selected').html()) == 0){
    corrob_util.$('#confirm_dialog').html('Please select some '+current+'.');
    corrob_util.$('#confirm_dialog').dialog({
      title : 'No '+current+' selected.',
      buttons : {
        "Confirm" : function (){
          corrob_util.$('#confirm_dialog').dialog("close");
        },
...
corrob_util.$('#view-placeholder-bulletin, #view-placeholder-actor').on('click','.upload-media',function(){
  corrob_util.$('#media_upload_dialog').dialog('open');
});
...
corrob_util.$('.do-expand').click(function(){
  ...
  if(corrob_util.$('#view-placeholder-'+current+' > .'+currentFirstUpper).hasClass('in-view')){
    ...
    corrob_util.$('.media-block').insertAfter(corrob_util.$('.is-sources.group'));
    return false;
  }
  ...
  corrob_util.$('#'+current+'-label-block').appendTo(corrob_util.$('#'+current+'-body-expand'));
...
corrob_util.$('.do-collapse').click(function(){
  ...
  if(corrob_util.$('#view-placeholder-'+current+' > .'+currentFirstUpper).hasClass('in-view')){
    ...
    corrob_util.$('#bulletin-view-body-block-ne').prepend(corrob_util.$('.media-block'));
    return false;
  }
